module.exports = function() {
  ...
  img.addEventListener(
    "mousemove",
    function(event) {
      if (lock || (event.clientX === lastCoords.x && event.clientY === lastCoords.y)) return;

      lock = true;

      var newCoords = { x: event.clientX, y: event.clientY };

      var angle = lastCoords.x === lastCoords.y ? Math.PI / 2 : Math.atan(Math.abs(lastCoords.y - newCoords.y) / Math.abs(lastCoords.x - newCoords.x));
      var real =
        (angle / (Math.PI / 2) +
          parseInt(
            Date.now()
